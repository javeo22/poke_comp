...
**Upcoming sessions**:
- **Session B (~5 hr)**: regional forms as separate roster entries (schema + UI dedup audit) + AI hallucination guardrails (schema-validated response + DB cross-check of cited moves/items + confidence badges).
- **Session C (~6 hr)**: matchup log visual + fields revamp + any remaining team-preview UX friction after Session A lands with the user.
