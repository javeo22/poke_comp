-- Migration: High Resolution Meta Trends
-- Date: 2026-06-06
...
BEGIN
    ...
    SELECT 
        ...
        50.0 as win_rate, 
        'Meta'::TEXT as role,
        COALESCE((SELECT jsonb_agg(x) FROM (SELECT * FROM jsonb_array_elements(ud.moves) LIMIT 3) x), '[]'::jsonb) as top_moves,
        COALESCE((SELECT jsonb_agg(x) FROM (SELECT * FROM jsonb_array_elements(ud.items) LIMIT 3) x), '[]'::jsonb) as top_items,
        COALESCE((SELECT jsonb_agg(x) FROM (SELECT * FROM jsonb_array_elements(ud.abilities) LIMIT 3) x), '[]'::jsonb) as top_abilities
    FROM usage_data ud
