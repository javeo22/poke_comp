...
async def test_approve_limitless_item(mock_supabase):
    ...
    # Mock production insert
    mock_query.insert.return_value.execute.return_value.data = [payload]
...
async def test_approve_pikalytics_item(mock_supabase):
    ...
    assert result["status"] == "APPROVED"
    # Verify production upsert
    mock_query.upsert.assert_called_with(payload)
