...
from app.services.retrieval import fetch_tournament_context, fetch_personal_context
...
def benchmark():
    ...
    try:
        res = supabase.table("pokemon").select("id, name").eq("champions_eligible", True).limit(6).execute()
        if not res.data:
        ...
        user_id = user_res.data[0]["user_id"] if user_res.data else "00000000-0000-0000-0000-000000000000"
    ...
    return {
        "tournament": {"p50": t_p50, "p95": t_p95},
        "personal": {"p50": p_p50, "p95": p_p95}
    }
